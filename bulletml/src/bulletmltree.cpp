...
std::string BulletMLNode::name2string[nameSize] = {
    "bullet",  "action",    "fire",       "changeDirection", "changeSpeed",
    "accel",   "wait",      "repeat",     "bulletRef",       "actionRef",
    "fireRef", "vanish",    "horizontal", "vertical",        "term",
    "times",   "direction", "speed",      "param",           "bulletml"};
...
BulletMLNode::Type BulletMLNode::string2type(const std::string& str) {
  if (str == "aim")
    ...
    BulletMLError::doAssert(std::string("BulletML parser: unknown type ") +
                            str + ".");
...
void BulletMLNode::getAllChildrenVec(Name name,
